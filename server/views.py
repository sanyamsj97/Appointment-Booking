...
def server(request):#this section for my appointment
	...
	if "server" == group_name:
		...
		return render(request, 'server.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/') 

def server_appointment_list(request):
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "server" == group_name:
		user_name=request.user.get_username()#Getting Username
		...
		return render(request, 'server_create_appointment.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')
...
def appointment_delete(request, id):
	...
	if "server" == group_name:
		...
		return redirect('http://127.0.0.1:8000/server/create_appointment/')
	else:
		return redirect('http://127.0.0.1:8000/')

def server_appointment_update(request,id):
	group_name=Group.objects.all().filter(user = request.user)# get logget user grouped name
	group_name=str(group_name[0]) # convert to string
	if "server" == group_name:
		user_name=request.user.get_username()#Getting Username
		...
		return render(request, 'server_appointment_update.html', appointments )
	else:
		return redirect('http://127.0.0.1:8000/')